...
class InpaintMobileVitPipeline(AbstractPipeline):
  ...
  def _forward_mobile_vit(self, inputs, time_steps, angles):
    return self.mobile_vit(inputs, time_steps, angles)
  ...
  def forward(self, **kwargs):
    ...
    output_mobile_vit = self._forward_mobile_vit(inputs=fill_images, time_steps=time_steps, angles=angles)
    output_unet = self._forward_unet(noisy_latents=noisy_latents, time_steps=time_steps, encoder_hidden_states=encoder_hidden_states, conditional_controls=output_mobile_vit)
    return output_unet
